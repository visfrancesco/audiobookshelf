...
@section('content')
<header class="page-header"><div><p class="eyebrow">YOUR SPACE TO LISTEN & LEARN</p><h1>{{ $library['name'] }}</h1><p class="muted">@if($view === 'stats')Your library at a glance @else{{ number_format($paginator->total()) }} {{ $view === 'items' ? ($paginator->total() === 1 ? 'item on your shelf' : 'items on your shelf') : str_replace('-', ' ', $view) }}@endif</p></div><div class="header-actions">
...
@if($library['mediaType'] === 'book' || !in_array($key, ['series', 'authors', 'narrators', 'collections']))<a href="{{ route('library', ['id' => $library['id'], 'view' => $key]) }}" wire:navigate aria-current="{{ $view === $key ? 'page' : 'false' }}" @class(['active' => $view === $key])>{{ $label }}</a>
@endif
@endforeach</nav>
@if($view === 'items')
...
@if(is_numeric($value))<article class="stat-card"><span>{{ ['totalAuthors' => 'Authors', 'totalGenres' => 'Genres', 'totalItems' => 'Library items', 'totalSize' => 'Storage used', 'totalDuration' => 'Listening time', 'numAudioTracks' => 'Audio tracks'][$key] ?? \Illuminate\Support\Str::headline($key) }}</span><strong>{{ match($key) { 'totalDuration' => \App\Services\MediaTime::duration($value), 'totalSize' => \App\Services\MediaSize::format($value), default => number_format($value) } }}</strong></article>
@endif
@endforeach</div>
